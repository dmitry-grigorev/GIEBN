...
#from networks.discrete import DiscreteBNGI
...
def construct_by_quantiles(data: pd.DataFrame, categoricals: list, scoring, max_cat=3, custom_mapper=None, directory='./'):
    ...
    bn = learn_bn(data_discretized_enc, categories, params, scoring=scoring, custom_mapper=custom_mapper, directory=directory)
...
def construct_by_uniform(data: pd.DataFrame, categoricals: list, scoring, max_cat=3, custom_mapper=None, directory='./'):
    data_discretized = data.copy(deep=True)
    conts = data.columns if categoricals is None else data.columns.difference(categoricals)
    for feat in conts:
...
def construct_by_kmeans(data: pd.DataFrame, categoricals: list, scoring, max_cat=3, custom_mapper=None, directory='./'):
    data_discretized = data.copy(deep=True)
    conts = data.columns if categoricals is None else data.columns.difference(categoricals)
...
def learn_bn(data_discretized_enc, categories, params, scoring, custom_mapper=None, directory='./'):
    all_edges = list()
    # Для демонстрации проблемы ансамблевое построение необязательно
    r = 1
    bn = DiscreteBNGI(outputdirectory=directory, custom_mapper=custom_mapper)
